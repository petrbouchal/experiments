...
obyv <- select(obyv, typuz_naz, nazev, uzcis, uzkod, vse5131, vse1111)
obyv <- mutate(obyv, stat = vse5131/vse1111)
obyv <- filter(obyv, typuz_naz=='okres')
obyv <- merge(obyv, uzemi, by=c('nazev','typuz_naz'),all.x=F, all.y=F)

unique(obyv$kodnuts)

# Load map shape data
setwd('~/Documents/Research/Geodata/CUZK//')
mapdata <- readOGR(dsn='.',layer='SPH_OKRES', encoding = 'LATIN2')
mapdata <- thinnedSpatialPoly(mapdata, 0.005, topologyPreserve = TRUE)

coordinates <- data.frame(long=coordinates(mapdata)[, 1],lat=coordinates(mapdata)[, 2])
coordinates[, 'NAZEV_LAU1'] <- as.character(mapdata@data[,'NAZEV_LAU1'])
coordinates[, 'KOD_LAU1'] <- as.character(mapdata@data[,'KOD_LAU1'])

mapdataset <- fortify(mapdata, region='KOD_LAU1')
...
plotdata <- merge(mapdataset, obyv, by.x='id',by.y='kodnuts',all.x=TRUE)
...
ggplot(plotdata, aes(x=long, y=lat)) +
  geom_polygon(aes(group=group, fill=stat, colour=stat)) +
  coord_map() +
  geom_text(aes(x=long, y=lat, label=NAZEV_LAU1), data=coordinates) +
...
all_values <- function(x) {
  ...
  paste0("<div style=\'font-family:sans-serif;\'>Kraj: ",
...
# LeafletR
library(leafletR)
q.dat <- toGeoJSON(mapdataset, dest=tempdir(), lat.lon=c("lat", 'long'))
leafletR::leaflet(q.dat)

# GGMap
library(ggmap)
ggmap(get_map(location = c(15.5,50), zoom = 7, source='stamen', maptype='toner-lite', color = 'bw')) +
  geom_polygon(aes(x=long, y=lat, group=group, fill=stat), colour='lightgrey',
